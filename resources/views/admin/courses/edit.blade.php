                        ...
                        <div class="card-body">
                            <form action="{{ route('courses.update', $course->id) }}" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="credits" id="credits" class="form-control" value="{{ old('credits', $course->credits) }}" min="1" required>
                                    @error('credits')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="lecturer_ids[]" id="lecturer_ids" class="form-control select2" multiple>
                                        @foreach ($lecturers as $lecturer)
                                            <option value="{{ $lecturer->id }}" {{ $course->lecturers->contains($lecturer->id) || in_array($lecturer->id, old('lecturer_ids', [])) ? 'selected' : '' }}>
                                                {{ $lecturer->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="faculty_ids[]" id="faculty_ids" class="form-control select2" multiple>
                                        @foreach ($faculties as $faculty)
                                            <option value="{{ $faculty->id }}" {{ $course->faculties->contains($faculty->id) || in_array($faculty->id, old('faculty_ids', [])) ? 'selected' : '' }}>
                                                {{ $faculty->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group text-right">
                                    ...
                                    <button type="submit" class="btn" style="background-color: #4B2E83; color: white;">Update Course</button>
                                </div>
                            </form>
                        </div>
                    </div>
