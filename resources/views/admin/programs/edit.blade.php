                ...
                <form action="{{ route('programs.update', $program) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="short_name" id="short_name" class="form-control"
                            value="{{ old('short_name', $program->short_name) }}" required placeholder="e.g., BScIT">
                        @error('short_name')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <input type="number" name="total_years" id="total_years" class="form-control"
                            value="{{ old('total_years', $program->total_years) }}" required min="1" max="10"
                            placeholder="e.g., 3">
                        @error('total_years')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                        ...
                        <select name="administrator_id" id="administrator_id" class="form-control select2" required>
                            ...
                            @foreach ($administrators as $admin)
                                <option value="{{ $admin->id }}"
                                    {{ old('administrator_id', $program->administrator_id) == $admin->id ? 'selected' : '' }}>
                                    {{ $admin->name }}
                                </option>
                            @endforeach
                        </select>
