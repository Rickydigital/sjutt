            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('timetable.update', $timetable->id) }}" method="POST" id="timetable-form">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="program_id" id="program_id" class="form-control select2" required>
                                        ...
                                        @foreach($programs as $program)
                                            <option value="{{ $program->id }}" {{ old('program_id', $timetable->faculty->program_id) == $program->id ? 'selected' : '' }}>
                                                {{ $program->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="time_start" id="time_start" class="form-control select2" required>
                                        ...
                                        @php
                                            ...
                                            while ($start <= $end) {
                                                $time = date('H:i', $start);
                                                $selected = old('time_start', $timetable->time_start) == $time ? 'selected' : '';
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="time_end" id="time_end" class="form-control select2" required>
                                        ...
                                        @php
                                            ...
                                            while ($start <= $end) {
                                                $time = date('H:i', $start);
                                                $selected = old('time_end', $timetable->time_end) == $time ? 'selected' : '';
                                                echo "<option value='$time' $selected>$time</option>";
                                                $start = strtotime('+30 minutes', $start);
                                            }
                                        @endphp
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="activity" id="activity" class="form-control" value="{{ old('activity', $timetable->activity) }}" required>
                                    @error('activity')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    <button type="submit" class="btn" style="background-color: #4B2E83; color: white;">
                                        <i class="fa fa-save mr-1"></i> Update Timetable Entry
                                    </button>
    ...
    <script>
        (function($j) {
            ...
            $j('#time_start').on('change', function() {
                ...
                const currentEndTime = endTime || '{{ old('time_end', $timetable->time_end) }}';
                ...
                $j.ajax({
                    ...
                    success: function(response) {
                        ...
                        $j.each(response.faculties, function(id, name) {
                            const selected = id == {{ old('faculty_id', $timetable->faculty_id) }} ? 'selected' : '';
                            $facultySelect.append(new Option(name, id, false, selected));
                        });
            ...
            $j('#faculty_id').on('change', function() {
                ...
                $j.ajax({
                    ...
                    success: function(response) {
                        ...
                        response.course_codes.forEach(function(code) {
                            const selected = code == '{{ old('course_code', $timetable->course_code) }}' ? 'selected' : '';
                            $courseSelect.append(new Option(code, code, false, selected));
                        });
                ...
                $j.ajax({
                    ...
                    success: function(response) {
                        $groupSelect.empty();
                        const currentGroups = '{{ old('group_selection', $timetable->group_selection) }}'.split(',');
                        if (response.groups.length > 1) {
                            response.groups.forEach(function(group) {
                                const selected = currentGroups.includes(group) ? 'selected' : '';
                                $groupSelect.append(new Option(group, group, false, selected));
                            });
                            $groupContainer.show();
                ...
                $j.ajax({
                    ...
                    success: function(response) {
                        ...
                        response.lecturers.forEach(function(lecturer) {
                            const selected = lecturer.id == {{ old('lecturer_id', $timetable->lecturer_id) ?? 'null' }} ? 'selected' : '';
                            $lecturerSelect.append(new Option(lecturer.name, lecturer.id, false, selected));
                        });
