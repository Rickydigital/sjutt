        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.add-timetable', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#modal_group_selection').empty().append(
                            '<option value="All Groups" class="select-all-option">All Groups</option>'
                            );
            ...
            ['#modal_course_code', '#edit_modal_course_code'].forEach(selector => {
                $(selector).on('change', function() {
                    ...
                    const $lecturerSelect = $(selector === '#modal_course_code' ?
                        '#modal_lecturer_id' : '#edit_modal_lecturer_id');
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            response.lecturers.forEach(lecturer => {
                                $lecturerSelect.append(new Option(lecturer.name,
                                    lecturer.id));
                            });
                            $lecturerSelect.prop('disabled', response.lecturers
                                .length === 0);
                        },
                        error: function(xhr) {
            ...
            $(document).on('click', '.edit-timetable', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        const timeStart = moment(data.time_start, ['H:mm:ss', 'H:mm']).format(
                            'HH:mm');
                        $('#edit_modal_time_start').val(timeStart);
                        $('#edit_modal_time_end').empty().append(
                            '<option value="">Select End Time</option>');
                        const timeSlots = @json($timeSlots);
                        const startIndex = timeSlots.indexOf(timeStart);
                        timeSlots.slice(startIndex + 1).forEach(time => {
                            // Format time_end to ensure HH:mm
                            const timeEnd = moment(data.time_end, ['H:mm:ss', 'H:mm'])
                                .format('HH:mm');
                            $('#edit_modal_time_end').append(new Option(time, time,
                                false, time === timeEnd));
                        });
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                $('#edit_modal_course_code').empty().append(
                                    '<option value="">Select Course Code</option>'
                                    );
                                response.course_codes.forEach(course => {
                                    $('#edit_modal_course_code').append(
                                        new Option(course.course_code,
                                            course.course_code, false,
                                            course.course_code === data
                                            .course_code));
                                });
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                $('#edit_modal_group_selection').empty().append(
                                    '<option value="All Groups" class="select-all-option">All Groups</option>'
                                    );
                                const selectedGroups = data.group_selection ? data
                                    .group_selection.split(',') : [];
                                response.groups.forEach(group => {
                                    $('#edit_modal_group_selection').append(
                                        new Option(group.group_name,
