...
<div id="modal-overlay"
    ...
    <div class="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate__animated animate__zoomIn animate__faster">

        <div class="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center mb-4 mx-auto text-indigo-600">
        ...
        <div class="mt-6">
            <label for="modal_form4_index" class="block text-sm font-bold text-gray-700 mb-2">
                Enter your Form Four Index Number
            </label>

            <input
                type="text"
                id="modal_form4_index"
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none uppercase"
                placeholder="Enter Form Four Index Number"
                autocomplete="off"
            >

            <p id="form4-error" class="hidden text-sm text-red-600 mt-2">
                Form Four Index Number is required.
            </p>
        </div>
...
<form id="voteForm" method="POST" action="{{ route('student.vote.store') }}" class="hidden">
    ...
    <input type="hidden" name="form4_index" id="vote_form4_index">
</form>
...
<script>
    ...
    function openModal(positionId, candidateId, candidateName) {
        ...
        document.getElementById('modal_form4_index').value = '';
        document.getElementById('form4-error').classList.add('hidden');
        ...
        setTimeout(() => {
            document.getElementById('modal_form4_index').focus();
        }, 150);
    }
    ...
    function processVote() {
        const form4Index = document.getElementById('modal_form4_index').value.trim();
        const errorBox = document.getElementById('form4-error');
        ...
        document.getElementById('vote_form4_index').value = form4Index;
